...
int get_middle_servo_current_angle();
//S1 and S2 regs
...
int main()
{
	...
	*(baseaddr_side_servos_write_current_angle) = 0x0000005A; // 90 degree
	sleep(5);
		...
		for (int index = 0; index < ARRAY_LENGTH * ARRAY_LENGTH; index++) {
			if(pair_array[index].input_1 == 0 && pair_array[index].input_2 == 0 && pair_array[index].occurrence == 0){
				...
				xil_printf("%d,%d,%d,%d,%d~",
						...
						get_middle_servo_current_angle(),
...
int get_middle_servo_current_angle(){
	if(*(baseaddr_middle_servo_stop) == 0x00000001){
			...
			return *baseaddr_middle_servo_current_angle;
		}else {
			return 0;
		}
}
